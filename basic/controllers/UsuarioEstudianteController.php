...
use app\models\UsuarioEstudianteSearch;
use app\models\User;
...
/**
 * UsuarioEstudianteController implementa las acciones CRUD para el modelo UsuarioEstudiante.
 */
class UsuarioEstudianteController extends Controller
{
    ...
    /**
     * Lista todos los modelos UsuarioEstudiante.
     * @return mixed
     */
    public function actionIndex()
    {
        $searchModel = new UsuarioEstudianteSearch();
    ...
    /**
     * Muestra un modelo UsuarioEstudiante específico.
     * @param integer $id
     * @return mixed
    ...
    /**
     * Crea un nuevo modelo UsuarioEstudiante.
    ...
    public function actionCreate()
    {
        ...
        if ($userModel->load(Yii::$app->request->post()) && $model->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                // Asignar el id_usuario al estudiante
                $model->id_usuario = $userModel->id_usuario;
                
                if (!$model->save()) {
                    throw new \Exception('Error al guardar el estudiante: ' . implode(', ', $model->getErrorSummary(true)));
                }
                
                $transaction->commit();
                Yii::$app->session->setFlash('success', 'Estudiante creado correctamente.');
    ...
    /**
     * Actualiza un modelo UsuarioEstudiante existente.
    ...
    public function actionUpdate($id)
    {
        ...
        if ($userModel->load(Yii::$app->request->post()) && $model->load(Yii::$app->request->post())) {
            ...
            try {
                ...
                if (!$model->save()) {
                    throw new \Exception('Error al actualizar el estudiante: ' . implode(', ', $model->getErrorSummary(true)));
                }
                
                $transaction->commit();
                Yii::$app->session->setFlash('success', 'Estudiante actualizado correctamente.');
                return $this->redirect(['view', 'id' => $model->id_estudiante]);
    ...
    /**
     * Elimina un modelo UsuarioEstudiante existente.
    ...
    public function actionDelete($id)
    {
        ...
        try {
            ...
            $transaction->commit();
            Yii::$app->session->setFlash('success', 'Estudiante eliminado correctamente.');
            
        } catch (\Exception $e) {
            $transaction->rollBack();
            Yii::$app->session->setFlash('error', 'Error al eliminar el estudiante: ' . $e->getMessage());
        }
    ...
    /**
     * Busca el modelo UsuarioEstudiante basado en su clave primaria.
     * Si el modelo no es encontrado, se lanzará una excepción 404 HTTP.
     * @param integer $id
     * @return UsuarioEstudiante el modelo cargado
     * @throws NotFoundHttpException si el modelo no es encontrado
     */
    protected function findModel($id)
    {
        if (($model = UsuarioEstudiante::findOne($id)) !== null) {
            return $model;
        }
